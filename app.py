        ...
        if st.button("تسجيل حساب | Register"):
            if new_user and new_password:
                try:
                    ...
                    conn.commit()
                    st.success("تم إنشاء الحساب بنجاح! يمكنك الآن تسجيل الدخول من القائمة الجانبية")
                except:
                    st.warning("اسم المستخدم هذا مسجل بالفعل، اختر اسماً آخر")
...
def main_app(username):
    ...
    texts = {
        "العربية": {
            ...
            "ai_msgs": ["يا بطل، الميزانية بتصوت! 📢 عدينا النص.", "😱 المصاريف كسبت! محتاجين توفير.", "ارحمني من الشوبينج النهاردة! ✨"]
        },
        "English": {
            ...
            "ai_msgs": ["Budget is screaming! 📢 Half gone.", "Expenses won 1-0! 😱 Needs control.", "Mercy please! Stop shopping. ✨"]
        }
    }
    t = texts[lang]
    st.title(t["title"])
    ...
    # جلب البيانات الخاصة باليوزر الحالي فقط
    ...
    if not df.empty:
        ...
        if total_spent > (income * 0.5):
            ...
            st.success("✅ Stable!" if lang == "English" else "✅ ميزانيتك مستقرة!")
